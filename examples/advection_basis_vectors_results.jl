...
function generate_basis_results(pde_function,pde_function_handle,opnn_pde_function_handle;random_integer = "None",kws...)
    ...
    opnn_output_width = Flux.outdims(trunk[end],ic)[1];
    ...
    pltfinal = plot(x,u_opnn_full[end,:],label="N = $(size(basis_full,2)) custom basis vector solution",legend=:outerbottom,foreground_color_legend = nothing,xlims=(x[1],x[end]),xlabel=L"x",ylabel=L"u(x)")
    ...
    plot!(pltfinal,x,u_fourier_M[end,:],label="M = 128 Fourier solution",linestyle=:dash)
    display(pltfinal)
    savefig(pltfinal, @sprintf("opnn_vs_exact_solution_end_%s_%i_%i.png",pde_function,args.num_sensors,args.N))
...
generate_basis_results("advection_equation",advection_pde!,opnn_advection_pde!;random_integer = "exact")
